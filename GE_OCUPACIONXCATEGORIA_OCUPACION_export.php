...
$page_layouts["GE_OCUPACIONXCATEGORIA_OCUPACION_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["OCUPACION_ID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["CATEGORIA_OCUPACION_ID"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["OCUPACION_ID"] = urldecode($arr[0]);
			$keys["CATEGORIA_OCUPACION_ID"] = urldecode($arr[1]);
			$selected_recs[] = $keys;
		}
	}
