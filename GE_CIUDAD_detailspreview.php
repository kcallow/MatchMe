...
require_once("include/GE_CIUDAD_variables.php");
...
$page_layouts["GE_CIUDAD_detailspreview"] = $layout;
...
if($mastertable == "GE.PAIS")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("PAIS_ID") . "=" . make_db_value("PAIS_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	PAIS_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("PAIS_ID", $data, $keylink);
			$row["PAIS_ID_value"] = $value;
			$format = $pSet->getViewFormat("PAIS_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("PAIS_ID")))
				$class = ' rnr-field-number';
			$row["PAIS_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_CIUDAD", "detailspreview"));
