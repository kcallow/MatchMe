...
if (@$_SESSION['webcharts']['settings']['title'] != "") {
	...
	$xt->assign("chart_title",", "."Título".": ".$title);
} else {
...
if (@$_SESSION['webcharts']['tables'][0] != "") {
	...
	$xt->assign("chart_table",", "."Tabla".": ".$stable);
} else {
			...
			$.ajax({
				...
				success: function(msg){
					if ( msg == "OK" ) {
						...
						if(this.id == "savebtn")
							$("#alert").html("<p>'."Ocurrión algún problema durante la grabación".'</p>").dialog("open");
						else
							$("#alert").html("<p>'."Ocurrió algún problema durante la previsualización".'</p>").dialog("open");
					}
				},
				error: function() {
					if(this.id == "savebtn")
...
$xt->assign("stacked", "<span id=\"td_stacked\" style=\"visibility:hidden\"><input type=\"checkbox\" ".$sstacked_checked." id=\"sstacked\" name=\"sstacked\" />&nbsp;"."100% Apilado"."</span>");
...
if ($_SESSION['webcharts']['chart_type']['type']=="2d_column" || $_SESSION['webcharts']['chart_type']['type']=="2d_bar" || $_SESSION['webcharts']['chart_type']['type']=="bubble" || $_SESSION['webcharts']['chart_type']['type']=="2d_pie" || $_SESSION['webcharts']['chart_type']['type']=="2d_doughnut") 
	$xt->assign("chart_is_3d","<input type=\"checkbox\" ".$is3d_checked." id=\"is3d\" name=\"is3d\">&nbsp;"."Gráfico 3D");
else 
...
if ($_SESSION['webcharts']['chart_type']['type']=="2d_column" || $_SESSION['webcharts']['chart_type']['type']=="2d_bar" || $_SESSION['webcharts']['chart_type']['type']=="area") 
	$xt->assign("chart_is_stacked","<input type=\"checkbox\" ".$isstacked_checked." id=\"isstacked\" name=\"isstacked\">&nbsp;"."Gráfico apilado");
else 
	$xt->assign("chart_is_stacked", '');	
if ( preg_match( "/bar/i", $_SESSION['webcharts']['chart_type']['type'] ) ) {
	$xt->assign("aqua", "Usar Estilo"." <select name=\"aqua\" id=\"aqua\">
		<option value=\"0\">"."Ninguno"."</option>
		<option value=\"1\">"."Agua clara"."</option>
		<option value=\"2\">"."Agua oscura"."</option>
	  </select>");
    $xt->assign("cview", "Tipo de gráfico"." <select name=\"cview\" id=\"cview\">
		<option value=\"0\">"."Columna"."</option>
		<option value=\"1\">"."Cono"."</option>
		<option value=\"2\">"."Cilindro"."</option>
		<option value=\"3\">"."Pirámide"."</option>
	  </select>");
} else {
	$xt->assign("aqua", '');
    $xt->assign("cview", '');	
...
if ( preg_match( "/line/i", $_SESSION['webcharts']['chart_type']['type'] ) ) {
	$xt->assign("line", "&nbsp;&nbsp;&nbsp;&nbsp;"."Estilo de línea"." <select name=\"linestyle\" id=\"linestyle\">
...
if ( preg_match( "/gauge/i", $_SESSION['webcharts']['chart_type']['type'] ) ) {
	$xt->assign("gauge", "Estilo de marcador"." <select name=\"gaugestyle\" id=\"gaugestyle\">
		<option value=\"0\">"."Círculo"."</option>
		<option value=\"1\">"."Horizontal"."</option>
		<option value=\"2\">"."Vertial"."</option>
	  </select>");
} else {
	$xt->assign("gauge", '');
}
