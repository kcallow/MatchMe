...
$page_layouts["GE_INTERESXCATEGORIA_INTERES_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["INTERES_ID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["CATEGORIA_INTERES_ID"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["INTERES_ID"] = urldecode($arr[0]);
			$keys["CATEGORIA_INTERES_ID"] = urldecode($arr[1]);
			$selected_recs[] = $keys;
		}
	}
