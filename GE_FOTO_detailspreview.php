...
require_once("include/GE_FOTO_variables.php");
...
$page_layouts["GE_FOTO_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["FOTO_ID"]));
	
	
	//	FOTO_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("FOTO_ID", $data, $keylink);
			$row["FOTO_ID_value"] = $value;
			$format = $pSet->getViewFormat("FOTO_ID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("FOTO_ID")))
				$class = ' rnr-field-number';
			$row["FOTO_ID_class"] = $class;
	//	RUTA - Database Image
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("RUTA", $data, $keylink);
			$row["RUTA_value"] = $value;
			$format = $pSet->getViewFormat("RUTA");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("RUTA")))
				$class = ' rnr-field-number';
			$row["RUTA_class"] = $class;
	//	USERNAME - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("GE_FOTO", "detailspreview"));
