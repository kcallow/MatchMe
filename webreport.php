...
$(document).ready(function(){
	...
	$(".del").click(function(){
		...
		var mess = (type == "report") ? "'."¿Realmente quire borrar el informe %s?".'" : "'."¿Realmente quiere borrar el gráfico %s?".'";
		mess=mess.replace("%s",title);
		$("#alert")
			.html("<p>"+mess+"</p>")
			.dialog("option", "buttons", {
				"'."No".'": function() { $(this).dialog("close"); },
				"'."Borrar artículos seleccionados".'": function() {
					$.ajax({
						type: "POST",
						...
						error: function() {
							$("#alert").html("<p>'."Inténtelo más tarde".'</p>").dialog("open");
						}
					});				
				}
			})
	...
	if(!count($arr_UserGroups))
	{
		$b_includes .= '$("#admin_sql").click(function(){
			$("#alert").dialog("option","title","'."Escriba la contraseña".'");
			$("#alert")
				.html("'."Contraseña".':&nbsp;<input type=password id=admin_password size=30 value=\"\">")
				.dialog("option", "buttons", {
						...
						$.ajax({
							...
							success: function(msg){
								if ( msg == "OK" ) {
									...
									$("#alert").dialog("option", "buttons", {"'."Cancelar".'": function() { $(this).dialog("close");}});
									$("#alert").html("<p>'."Cambiar contraseña".'</p>").dialog("open");
								}
							}
						});				
					}
...
$arr_tables_custom = DBGetTablesListByGroup("custom");
	foreach ( $arrSharedReports as $rpt ) {
		...
		$shared_reports .= '<a class="action view" type="report" href="#">['."Ver".']</a>';
		/*
		 * If there's no login page
		 */
		if(trim($rpt["owner"]) == trim(@$_SESSION["UserID"]))
		{		
			...
			$shared_reports .= '<a class="action del" type="report" href="#">['."Borrar artículos seleccionados".']</a>';
		}
		$shared_reports .= '</span>';
		$shared_reports .= '</div>'."\r\n";
	}
	foreach ( $arrSharedCharts as $chart ) {
		...
		$shared_charts .= '<a class="action view" type="chart" href="#">['."Ver".']</a>';
		/*
		 * If there's no login page
		 */
		if(trim($chart["owner"]) == trim(@$_SESSION["UserID"]))
		{
			if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
				$shared_charts .= '<a class="action edit" type="chart" href="#">['."Editar".']</a>';
			$shared_charts .= '<a class="action del" type="chart" href="#">['."Borrar artículos seleccionados".']</a>';
		}
		$shared_charts .= '</span>';	
		$shared_charts .= '</div>'."\r\n";
	}
...
foreach ( $arrPrivateReports as $rpt ) {
	if ( $rpt["status"] == "public" ) {
		...
		$private_reports .= '<a class="action view" type="report" href="#">['."Ver".']</a>';
		if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
			$private_reports .= '<a class="action edit" type="report" href="#">['."Editar".']</a>';
		$private_reports .= '<a class="action del" type="report" href="#">['."Borrar artículos seleccionados".']</a>';
...
foreach ( $arrPrivateCharts as $chart ) {
	if ( $chart["status"] == "public" ) {
		...
		$private_charts .= '<a class="action view" type="chart" href="#">['."Ver".']</a>';
		if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
			$private_charts .= '<a class="action edit" type="chart" href="#">['."Editar".']</a>';
		$private_charts .= '<a class="action del" type="chart" href="#">['."Borrar artículos seleccionados".']</a>';
		$private_charts .= '</span>';		
		$private_charts .= '</div>'."\r\n";
	} else {
		$private_charts .= '<div style="margin-bottom:5px;">';
...
{
	$create_butt="<a id=\"admin_page\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"admin_page\" href=\"#\">"."Area Admin"."</a>";
	$xt->assign("admin_page", $create_butt);
}
if(!$wr_is_standalone)
{
	$create_butt="<a id=\"return_app\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"return_app\" href=\"#\">"."Volver al programa principal"."</a>";
	$xt->assign("back_to_app", $create_butt);
}
if($wr_is_standalone && isWRAdmin())	
{
	$create_butt="<a id=\"users_list\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"users_list\" href=\"#\">"."Lista de usuarios"."</a>";
	$xt->assign("users_list_page", $create_butt);
}

if(isWRAdmin() || !count($arr_UserGroups))
{
	$create_butt="<a id=\"admin_sql\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"admin_sql\" href=\"#\">"."Consulta SQL"."</a>";
...
if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
{                 
	$create_butt="<a id=\"report_createbtn\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"report_createbtn\" wtype=\"report\" href=\"#\">"."Crear informe"."</a>";
    $create_butt.="<a id=\"chart_createbtn\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"chart_createbtn\" wtype=\"chart\" href=\"#\">"."Crear gráfico"."</a>";
	$xt->assign("create_report_chart", $create_butt);
}
else
{	
	if($wr_is_standalone && !isWRAdmin())
		$xt->assign("create_report_chart", "<b>"."No tiene permisos para crear informes y gráficos. Consulte con su administrador."."</b>");
}
