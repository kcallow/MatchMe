...
require_once("include/GE_COLOR_PELO_variables.php");
...
$page_layouts["GE_COLOR_PELO_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["COLOR_PELO_ID"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["COLOR_PELO_ID"] = urldecode($arr[0]);
			$selected_recs[] = $keys;
		}
	}
