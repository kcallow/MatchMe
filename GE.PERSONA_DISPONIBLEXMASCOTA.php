 ...
 */
    ...
    class GE_PERSONA_DISPONIBLEXMASCOTAPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, true);
            $this->dataset->AddLookupField('MASCOTA_ID', 'GE.MASCOTA', new IntegerField('MASCOTA_ID'), new StringField('TIPO', 'LA1', 'LT1'), 'LT1');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_PERSONA_DISPONIBLEXMASCOTAssearch', $this->dataset,
                array('USERNAME', 'ES_REAL', 'LA1', 'CANTIDAD', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('USERNAME'), $this->RenderText('ES REAL'), $this->RenderText('MASCOTA ID'), $this->RenderText('CANTIDAD'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_PERSONA_DISPONIBLEXMASCOTAasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('MASCOTA_ID', $this->RenderText('MASCOTA ID'), $lookupDataset, 'MASCOTA_ID', 'TIPO', false, 8));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('LA1', 'MASCOTA ID', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'MASCOTA_ID', 
                $editor, 
                $this->dataset, 'MASCOTA_ID', 'TIPO', $lookupDataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_PERSONA_DISPONIBLEXMASCOTA_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_PERSONA_DISPONIBLEXMASCOTAGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_PERSONA_DISPONIBLEXMASCOTAPage("GE.PERSONA_DISPONIBLEXMASCOTA.php", "GE_PERSONA_DISPONIBLEXMASCOTA", GetCurrentUserGrantForDataSource("GE.PERSONA_DISPONIBLEXMASCOTA"), 'UTF-8');
        $Page->SetShortCaption('PERSONA DISPONIBLEXMASCOTA');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('PERSONA DISPONIBLEXMASCOTA');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.PERSONA_DISPONIBLEXMASCOTA"));
