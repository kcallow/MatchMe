 ...
 */
    ...
    class GE_EDADPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '"GE"."EDAD"');
            $field = new IntegerField('EDAD_ID');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_EDADssearch', $this->dataset,
                array('EDAD_ID', 'RANGO', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('EDAD ID'), $this->RenderText('RANGO'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_EDADasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('EDAD_ID', $this->RenderText('EDAD ID')));
        ...
        {
            //
            // View column for EDAD_ID field
            //
            $column = new TextViewColumn('EDAD_ID', 'EDAD ID', $this->dataset);
        ...
        {
            //
            // Edit column for EDAD_ID field
            //
            $editor = new TextEdit('edad_id_edit');
            $editColumn = new CustomEditColumn('EDAD ID', 'EDAD_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_EDAD_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_EDADGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_EDADPage("GE.EDAD.php", "GE_EDAD", GetCurrentUserGrantForDataSource("GE.EDAD"), 'UTF-8');
        $Page->SetShortCaption('EDAD');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('EDAD');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.EDAD"));
