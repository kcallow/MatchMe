...
require_once("include/GE_VISITA_variables.php");
...
$page_layouts["GE_VISITA_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("USERNAME_VISITANTE") . "=" . make_db_value("USERNAME_VISITANTE",$_SESSION[$strTableName."_masterkey1"]);
		$where.=" and ";
	$where .= $pageObject->getFieldSQLDecrypt("USERNAME_VISITADA") . "=" . make_db_value("USERNAME_VISITADA",$_SESSION[$strTableName."_masterkey2"]);
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["FEC_VISITA"]));
		$keylink.="&key2=".runner_htmlspecialchars(rawurlencode(@$data["USERNAME_VISITANTE"]));
		$keylink.="&key3=".runner_htmlspecialchars(rawurlencode(@$data["USERNAME_VISITADA"]));
	
	
	//	FEC_VISITA - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("FEC_VISITA", $data, $keylink);
			$row["FEC_VISITA_value"] = $value;
			$format = $pSet->getViewFormat("FEC_VISITA");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("FEC_VISITA")))
				$class = ' rnr-field-number';
			$row["FEC_VISITA_class"] = $class;
	//	USERNAME_VISITANTE - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("USERNAME_VISITANTE", $data, $keylink);
			$row["USERNAME_VISITANTE_value"] = $value;
			$format = $pSet->getViewFormat("USERNAME_VISITANTE");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("USERNAME_VISITANTE")))
				$class = ' rnr-field-number';
			$row["USERNAME_VISITANTE_class"] = $class;
	//	USERNAME_VISITADA - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("USERNAME_VISITADA", $data, $keylink);
			$row["USERNAME_VISITADA_value"] = $value;
			$format = $pSet->getViewFormat("USERNAME_VISITADA");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("USERNAME_VISITADA")))
				$class = ' rnr-field-number';
			$row["USERNAME_VISITADA_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_VISITA", "detailspreview"));
