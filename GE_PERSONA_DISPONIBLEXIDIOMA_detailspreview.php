...
require_once("include/GE_PERSONA_DISPONIBLEXIDIOMA_variables.php");
...
$page_layouts["GE_PERSONA_DISPONIBLEXIDIOMA_detailspreview"] = $layout;
...
if($mastertable == "GE.IDIOMA")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("IDIOMA_ID") . "=" . make_db_value("IDIOMA_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	IDIOMA_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("IDIOMA_ID", $data, $keylink);
			$row["IDIOMA_ID_value"] = $value;
			$format = $pSet->getViewFormat("IDIOMA_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("IDIOMA_ID")))
				$class = ' rnr-field-number';
			$row["IDIOMA_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_PERSONA_DISPONIBLEXIDIOMA", "detailspreview"));
