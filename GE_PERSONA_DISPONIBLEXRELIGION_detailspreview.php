...
require_once("include/GE_PERSONA_DISPONIBLEXRELIGION_variables.php");
...
$page_layouts["GE_PERSONA_DISPONIBLEXRELIGION_detailspreview"] = $layout;
...
if($mastertable == "GE.RELIGION")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("RELIGION_ID") . "=" . make_db_value("RELIGION_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	RELIGION_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("RELIGION_ID", $data, $keylink);
			$row["RELIGION_ID_value"] = $value;
			$format = $pSet->getViewFormat("RELIGION_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("RELIGION_ID")))
				$class = ' rnr-field-number';
			$row["RELIGION_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_PERSONA_DISPONIBLEXRELIGION", "detailspreview"));
