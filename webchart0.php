...
$b_includes .= alertDialog();
	...
	$b_includes.='$("#radio_db,#radio_project,#radio_custom").click(function(){
		...
		if($(this).attr("id")=="radio_db")
		{
			...
			li_selected="db";
			$("#tl").html("'."Seleccione la tabla de la que quiere crear el gráfico:".'");
	...
	$b_includes .= '
			...
			$("#tl").html("'."Seleccione la consulta de la que quiere crear el gráfico:".'");
			NEXT_PAGE_URL = "'.GetTableLink("webchart3").'";
			PREV_PAGE_URL = "'.GetTableLink("webreport").'";';
	$b_includes .= '
			$("td[id=row1], td[id=row2]").hide();'."\r\n";
...
if(postvalue("sqlname"))
	$b_includes .= '
					...
					$("#radio_custom").click();
					li_selected="custom";
					$("#tl").html("'."Seleccione la consulta de la que crear el informe:".'");
					';
else
{	
	if(is_wr_db() && count($arr_tables_db))
...
if (@$root['settings']['title'] != "") {
	$xt->assign("chart_title",", "."Título".": ".@$root['settings']['title']);
} else {
