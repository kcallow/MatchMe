 ...
 */
    ...
    class GE_PERSONA_DISPONIBLEXRELIGIONPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, true);
            $field = new IntegerField('RELIGION_ID');
            $this->dataset->AddField($field, true);
            ...
            $this->dataset->AddField($field, true);
            $this->dataset->AddLookupField('RELIGION_ID', 'GE.RELIGION', new IntegerField('RELIGION_ID'), new StringField('NOMBRE', 'LA1', 'LT1'), 'LT1');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_PERSONA_DISPONIBLEXRELIGIONssearch', $this->dataset,
                array('USERNAME', 'ES_REAL', 'LA1', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('USERNAME'), $this->RenderText('ES REAL'), $this->RenderText('RELIGION ID'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_PERSONA_DISPONIBLEXRELIGIONasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('RELIGION_ID', $this->RenderText('RELIGION ID'), $lookupDataset, 'RELIGION_ID', 'NOMBRE', false, 8));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('LA1', 'RELIGION ID', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for RELIGION_ID field
            //
            $editor = new ComboBox('religion_id_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."RELIGION"');
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'RELIGION_ID', 
                $editor, 
                $this->dataset, 'RELIGION_ID', 'NOMBRE', $lookupDataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_PERSONA_DISPONIBLEXRELIGION_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_PERSONA_DISPONIBLEXRELIGIONGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_PERSONA_DISPONIBLEXRELIGIONPage("GE.PERSONA_DISPONIBLEXRELIGION.php", "GE_PERSONA_DISPONIBLEXRELIGION", GetCurrentUserGrantForDataSource("GE.PERSONA_DISPONIBLEXRELIGION"), 'UTF-8');
        $Page->SetShortCaption('GE.PERSONA DISPONIBLEXRELIGION');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.PERSONA DISPONIBLEXRELIGION');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.PERSONA_DISPONIBLEXRELIGION"));
