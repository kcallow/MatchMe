 ...
 */
    ...
    class GE_INTERESPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_INTERESssearch', $this->dataset,
                array('INTERES_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('INTERES ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_INTERESasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('INTERES_ID', $this->RenderText('INTERES ID')));
        ...
        {
            //
            // View column for INTERES_ID field
            //
            $column = new TextViewColumn('INTERES_ID', 'INTERES ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('interes_id_edit');
            $editColumn = new CustomEditColumn('INTERES ID', 'INTERES_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_INTERES_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_INTERESGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_INTERESPage("GE.INTERES.php", "GE_INTERES", GetCurrentUserGrantForDataSource("GE.INTERES"), 'UTF-8');
        $Page->SetShortCaption('GE.INTERES');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.INTERES');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.INTERES"));
