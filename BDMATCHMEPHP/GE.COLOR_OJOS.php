 ...
 */
    ...
    class GE_COLOR_OJOSPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_COLOR_OJOSssearch', $this->dataset,
                array('COLOR_OJOS_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('COLOR OJOS ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_COLOR_OJOSasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('COLOR_OJOS_ID', $this->RenderText('COLOR OJOS ID')));
        ...
        {
            //
            // View column for COLOR_OJOS_ID field
            //
            $column = new TextViewColumn('COLOR_OJOS_ID', 'COLOR OJOS ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('color_ojos_id_edit');
            $editColumn = new CustomEditColumn('COLOR OJOS ID', 'COLOR_OJOS_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_COLOR_OJOS_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_COLOR_OJOSGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_COLOR_OJOSPage("GE.COLOR_OJOS.php", "GE_COLOR_OJOS", GetCurrentUserGrantForDataSource("GE.COLOR_OJOS"), 'UTF-8');
        $Page->SetShortCaption('GE.COLOR OJOS');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.COLOR OJOS');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.COLOR_OJOS"));
