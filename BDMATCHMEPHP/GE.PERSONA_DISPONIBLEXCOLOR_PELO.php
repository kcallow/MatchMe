 ...
 */
    ...
    class GE_PERSONA_DISPONIBLEXCOLOR_PELOPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, true);
            $field = new IntegerField('COLOR_PELO_ID');
            $this->dataset->AddField($field, true);
            ...
            $this->dataset->AddField($field, true);
            $this->dataset->AddLookupField('COLOR_PELO_ID', 'GE.COLOR_PELO', new IntegerField('COLOR_PELO_ID'), new StringField('NOMBRE', 'LA1', 'LT1'), 'LT1');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_PERSONA_DISPONIBLEXCOLOR_PELOssearch', $this->dataset,
                array('USERNAME', 'ES_REAL', 'LA1', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('USERNAME'), $this->RenderText('ES REAL'), $this->RenderText('COLOR PELO ID'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_PERSONA_DISPONIBLEXCOLOR_PELOasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('COLOR_PELO_ID', $this->RenderText('COLOR PELO ID'), $lookupDataset, 'COLOR_PELO_ID', 'NOMBRE', false, 8));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('LA1', 'COLOR PELO ID', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for COLOR_PELO_ID field
            //
            $editor = new ComboBox('color_pelo_id_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."COLOR_PELO"');
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'COLOR_PELO_ID', 
                $editor, 
                $this->dataset, 'COLOR_PELO_ID', 'NOMBRE', $lookupDataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_PERSONA_DISPONIBLEXCOLOR_PELO_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_PERSONA_DISPONIBLEXCOLOR_PELOGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_PERSONA_DISPONIBLEXCOLOR_PELOPage("GE.PERSONA_DISPONIBLEXCOLOR_PELO.php", "GE_PERSONA_DISPONIBLEXCOLOR_PELO", GetCurrentUserGrantForDataSource("GE.PERSONA_DISPONIBLEXCOLOR_PELO"), 'UTF-8');
        $Page->SetShortCaption('GE.PERSONA DISPONIBLEXCOLOR PELO');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.PERSONA DISPONIBLEXCOLOR PELO');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.PERSONA_DISPONIBLEXCOLOR_PELO"));
