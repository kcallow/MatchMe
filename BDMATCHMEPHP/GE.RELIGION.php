 ...
 */
    ...
    class GE_RELIGIONPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_RELIGIONssearch', $this->dataset,
                array('RELIGION_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('RELIGION ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_RELIGIONasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('RELIGION_ID', $this->RenderText('RELIGION ID')));
        ...
        {
            //
            // View column for RELIGION_ID field
            //
            $column = new TextViewColumn('RELIGION_ID', 'RELIGION ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('religion_id_edit');
            $editColumn = new CustomEditColumn('RELIGION ID', 'RELIGION_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_RELIGION_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_RELIGIONGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_RELIGIONPage("GE.RELIGION.php", "GE_RELIGION", GetCurrentUserGrantForDataSource("GE.RELIGION"), 'UTF-8');
        $Page->SetShortCaption('GE.RELIGION');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.RELIGION');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.RELIGION"));
