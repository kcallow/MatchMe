 ...
 */
    ...
    class GE_WINKPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_WINKssearch', $this->dataset,
                array('FEC_WINK', 'LA1', 'LA2', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('FEC WINK'), $this->RenderText('USERNAME EMISOR'), $this->RenderText('USERNAME RECEPTOR'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_WINKasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateDateTimeSearchInput('FEC_WINK', $this->RenderText('FEC WINK'), 'Y-m-d'));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('USERNAME_EMISOR', $this->RenderText('USERNAME EMISOR'), $lookupDataset, 'USERNAME', 'ES_ADMIN', false, 8));
            ...
            $lookupDataset->AddField($field, false);
            $lookupDataset->SetOrderBy('ES_ADMIN', GetOrderTypeAsSQL(otAscending));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('USERNAME_RECEPTOR', $this->RenderText('USERNAME RECEPTOR'), $lookupDataset, 'USERNAME', 'ES_ADMIN', false, 8));
        ...
        public function GetModalGridDeleteHandler() { return 'GE_WINK_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_WINKGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_WINKPage("GE.WINK.php", "GE_WINK", GetCurrentUserGrantForDataSource("GE.WINK"), 'UTF-8');
        $Page->SetShortCaption('GE.WINK');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.WINK');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.WINK"));
