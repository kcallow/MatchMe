 ...
 */
    ...
    class GE_GENEROPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_GENEROssearch', $this->dataset,
                array('GENERO_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('GENERO ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_GENEROasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('GENERO_ID', $this->RenderText('GENERO ID')));
        ...
        {
            //
            // View column for GENERO_ID field
            //
            $column = new TextViewColumn('GENERO_ID', 'GENERO ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('genero_id_edit');
            $editColumn = new CustomEditColumn('GENERO ID', 'GENERO_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_GENERO_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_GENEROGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_GENEROPage("GE.GENERO.php", "GE_GENERO", GetCurrentUserGrantForDataSource("GE.GENERO"), 'UTF-8');
        $Page->SetShortCaption('GE.GENERO');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.GENERO');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.GENERO"));
