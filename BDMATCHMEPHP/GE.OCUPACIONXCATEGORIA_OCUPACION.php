 ...
 */
    ...
    class GE_OCUPACIONXCATEGORIA_OCUPACIONPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new IntegerField('OCUPACION_ID');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new IntegerField('CATEGORIA_OCUPACION_ID');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            ...
            $this->dataset->AddField($field, false);
            $this->dataset->AddLookupField('OCUPACION_ID', 'GE.OCUPACION', new IntegerField('OCUPACION_ID'), new StringField('NOMBRE', 'LA1', 'LT1'), 'LT1');
            $this->dataset->AddLookupField('CATEGORIA_OCUPACION_ID', 'GE.CATEGORIA_OCUPACION', new IntegerField('CATEGORIA_OCUPACION_ID'), new StringField('NOMBRE', 'LA2', 'LT2'), 'LT2');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_OCUPACIONXCATEGORIA_OCUPACIONssearch', $this->dataset,
                array('LA1', 'LA2', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('OCUPACION ID'), $this->RenderText('CATEGORIA OCUPACION ID'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_OCUPACIONXCATEGORIA_OCUPACIONasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('OCUPACION_ID', $this->RenderText('OCUPACION ID'), $lookupDataset, 'OCUPACION_ID', 'NOMBRE', false, 8));
            
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."CATEGORIA_OCUPACION"');
            $field = new IntegerField('CATEGORIA_OCUPACION_ID');
            $field->SetIsNotNull(true);
            $lookupDataset->AddField($field, true);
            $field = new StringField('NOMBRE');
            ...
            $lookupDataset->AddField($field, false);
            $lookupDataset->SetOrderBy('NOMBRE', GetOrderTypeAsSQL(otAscending));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('CATEGORIA_OCUPACION_ID', $this->RenderText('CATEGORIA OCUPACION ID'), $lookupDataset, 'CATEGORIA_OCUPACION_ID', 'NOMBRE', false, 8));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('LA1', 'OCUPACION ID', $this->dataset);
            ...
            // View column for NOMBRE field
            //
            $column = new TextViewColumn('LA2', 'CATEGORIA OCUPACION ID', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        {
            //
            // Edit column for OCUPACION_ID field
            //
            $editor = new ComboBox('ocupacion_id_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."OCUPACION"');
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'OCUPACION_ID', 
                $editor, 
                $this->dataset, 'OCUPACION_ID', 'NOMBRE', $lookupDataset);
            ...
            //
            // Edit column for CATEGORIA_OCUPACION_ID field
            //
            $editor = new ComboBox('categoria_ocupacion_id_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."CATEGORIA_OCUPACION"');
            ...
            $lookupDataset->SetOrderBy('NOMBRE', GetOrderTypeAsSQL(otAscending));
            $editColumn = new LookUpEditColumn(
                'CATEGORIA OCUPACION ID', 
                'CATEGORIA_OCUPACION_ID', 
                $editor, 
                $this->dataset, 'CATEGORIA_OCUPACION_ID', 'NOMBRE', $lookupDataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_OCUPACIONXCATEGORIA_OCUPACION_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_OCUPACIONXCATEGORIA_OCUPACIONGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_OCUPACIONXCATEGORIA_OCUPACIONPage("GE.OCUPACIONXCATEGORIA_OCUPACION.php", "GE_OCUPACIONXCATEGORIA_OCUPACION", GetCurrentUserGrantForDataSource("GE.OCUPACIONXCATEGORIA_OCUPACION"), 'UTF-8');
        $Page->SetShortCaption('GE.OCUPACIONXCATEGORIA OCUPACION');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.OCUPACIONXCATEGORIA OCUPACION');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.OCUPACIONXCATEGORIA_OCUPACION"));
