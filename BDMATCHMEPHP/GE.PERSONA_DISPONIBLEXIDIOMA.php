 ...
 */
    ...
    class GE_PERSONA_DISPONIBLEXIDIOMAPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, true);
            $field = new IntegerField('IDIOMA_ID');
            $this->dataset->AddField($field, true);
            ...
            $this->dataset->AddField($field, true);
            $this->dataset->AddLookupField('IDIOMA_ID', 'GE.IDIOMA', new IntegerField('IDIOMA_ID'), new StringField('NOMBRE', 'LA1', 'LT1'), 'LT1');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_PERSONA_DISPONIBLEXIDIOMAssearch', $this->dataset,
                array('USERNAME', 'ES_REAL', 'LA1', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('USERNAME'), $this->RenderText('ES REAL'), $this->RenderText('IDIOMA ID'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_PERSONA_DISPONIBLEXIDIOMAasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('IDIOMA_ID', $this->RenderText('IDIOMA ID'), $lookupDataset, 'IDIOMA_ID', 'NOMBRE', false, 8));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('LA1', 'IDIOMA ID', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for IDIOMA_ID field
            //
            $editor = new ComboBox('idioma_id_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."IDIOMA"');
            ...
            $editColumn = new LookUpEditColumn(
                ...
                'IDIOMA_ID', 
                $editor, 
                $this->dataset, 'IDIOMA_ID', 'NOMBRE', $lookupDataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_PERSONA_DISPONIBLEXIDIOMA_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_PERSONA_DISPONIBLEXIDIOMAGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_PERSONA_DISPONIBLEXIDIOMAPage("GE.PERSONA_DISPONIBLEXIDIOMA.php", "GE_PERSONA_DISPONIBLEXIDIOMA", GetCurrentUserGrantForDataSource("GE.PERSONA_DISPONIBLEXIDIOMA"), 'UTF-8');
        $Page->SetShortCaption('GE.PERSONA DISPONIBLEXIDIOMA');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.PERSONA DISPONIBLEXIDIOMA');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.PERSONA_DISPONIBLEXIDIOMA"));
