 ...
 */
    ...
    class GE_IDIOMAPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_IDIOMAssearch', $this->dataset,
                array('IDIOMA_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('IDIOMA ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_IDIOMAasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('IDIOMA_ID', $this->RenderText('IDIOMA ID')));
        ...
        {
            //
            // View column for IDIOMA_ID field
            //
            $column = new TextViewColumn('IDIOMA_ID', 'IDIOMA ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('idioma_id_edit');
            $editColumn = new CustomEditColumn('IDIOMA ID', 'IDIOMA_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_IDIOMA_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_IDIOMAGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_IDIOMAPage("GE.IDIOMA.php", "GE_IDIOMA", GetCurrentUserGrantForDataSource("GE.IDIOMA"), 'UTF-8');
        $Page->SetShortCaption('GE.IDIOMA');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.IDIOMA');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.IDIOMA"));
