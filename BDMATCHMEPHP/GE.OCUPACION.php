 ...
 */
    ...
    class GE_OCUPACIONPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_OCUPACIONssearch', $this->dataset,
                array('OCUPACION_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('OCUPACION ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_OCUPACIONasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('OCUPACION_ID', $this->RenderText('OCUPACION ID')));
        ...
        {
            //
            // View column for OCUPACION_ID field
            //
            $column = new TextViewColumn('OCUPACION_ID', 'OCUPACION ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('ocupacion_id_edit');
            $editColumn = new CustomEditColumn('OCUPACION ID', 'OCUPACION_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_OCUPACION_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_OCUPACIONGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_OCUPACIONPage("GE.OCUPACION.php", "GE_OCUPACION", GetCurrentUserGrantForDataSource("GE.OCUPACION"), 'UTF-8');
        $Page->SetShortCaption('GE.OCUPACION');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.OCUPACION');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.OCUPACION"));
