 ...
 */
    ...
    class GE_VISITAPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '"GE"."VISITA"');
            $field = new DateField('FEC_VISITA');
            $this->dataset->AddField($field, false);
            $field = new StringField('USERNAME_VISITANTE');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('USERNAME_VISITADA');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            ...
            $this->dataset->AddField($field, false);
            $this->dataset->AddLookupField('USERNAME_VISITANTE', 'GE.PERSONA', new StringField('USERNAME'), new StringField('ES_ADMIN', 'LA1', 'LT1'), 'LT1');
            $this->dataset->AddLookupField('USERNAME_VISITADA', 'GE.PERSONA', new StringField('USERNAME'), new StringField('ES_ADMIN', 'LA2', 'LT2'), 'LT2');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_VISITAssearch', $this->dataset,
                array('FEC_VISITA', 'LA1', 'LA2', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('FEC VISITA'), $this->RenderText('USERNAME VISITANTE'), $this->RenderText('USERNAME VISITADA'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_VISITAasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateDateTimeSearchInput('FEC_VISITA', $this->RenderText('FEC VISITA'), 'Y-m-d'));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('USERNAME_VISITANTE', $this->RenderText('USERNAME VISITANTE'), $lookupDataset, 'USERNAME', 'ES_ADMIN', false, 8));
            ...
            $lookupDataset->AddField($field, false);
            $lookupDataset->SetOrderBy('ES_ADMIN', GetOrderTypeAsSQL(otAscending));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('USERNAME_VISITADA', $this->RenderText('USERNAME VISITADA'), $lookupDataset, 'USERNAME', 'ES_ADMIN', false, 8));
        ...
        {
            //
            // View column for FEC_VISITA field
            //
            $column = new DateTimeViewColumn('FEC_VISITA', 'FEC VISITA', $this->dataset);
            ...
            //
            $column = new TextViewColumn('LA1', 'USERNAME VISITANTE', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            // View column for ES_ADMIN field
            //
            $column = new TextViewColumn('LA2', 'USERNAME VISITADA', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        {
            //
            // Edit column for FEC_VISITA field
            //
            $editor = new DateTimeEdit('fec_visita_edit', false, 'Y-m-d H:i:s', GetFirstDayOfWeek());
            $editColumn = new CustomEditColumn('FEC VISITA', 'FEC_VISITA', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for USERNAME_VISITANTE field
            //
            $editor = new ComboBox('username_visitante_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."PERSONA"');
            ...
            //
            // Edit column for USERNAME_VISITADA field
            //
            $editor = new ComboBox('username_visitada_edit', $this->GetLocalizerCaptions()->GetMessageString('PleaseSelect'));
            $lookupDataset = new TableDataset(
                new OracleConnectionFactory(),
                GetConnectionOptions(),
                '"GE"."PERSONA"');
        ...
        public function GetModalGridDeleteHandler() { return 'GE_VISITA_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_VISITAGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_VISITAPage("GE.VISITA.php", "GE_VISITA", GetCurrentUserGrantForDataSource("GE.VISITA"), 'UTF-8');
        $Page->SetShortCaption('GE.VISITA');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.VISITA');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.VISITA"));
