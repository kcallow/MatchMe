 ...
 */
    ...
    class GE_CATEGORIA_INTERESPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_CATEGORIA_INTERESssearch', $this->dataset,
                array('CATEGORIA_INTERES_ID', 'NOMBRE', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('CATEGORIA INTERES ID'), $this->RenderText('NOMBRE'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_CATEGORIA_INTERESasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('CATEGORIA_INTERES_ID', $this->RenderText('CATEGORIA INTERES ID')));
        ...
        {
            //
            // View column for CATEGORIA_INTERES_ID field
            //
            $column = new TextViewColumn('CATEGORIA_INTERES_ID', 'CATEGORIA INTERES ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('categoria_interes_id_edit');
            $editColumn = new CustomEditColumn('CATEGORIA INTERES ID', 'CATEGORIA_INTERES_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_CATEGORIA_INTERES_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_CATEGORIA_INTERESGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_CATEGORIA_INTERESPage("GE.CATEGORIA_INTERES.php", "GE_CATEGORIA_INTERES", GetCurrentUserGrantForDataSource("GE.CATEGORIA_INTERES"), 'UTF-8');
        $Page->SetShortCaption('GE.CATEGORIA INTERES');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.CATEGORIA INTERES');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.CATEGORIA_INTERES"));
