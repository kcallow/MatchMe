 ...
 */
    ...
    class GE_FUMADORPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_FUMADORssearch', $this->dataset,
                array('FUMADOR_ID', 'TIPO', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('FUMADOR ID'), $this->RenderText('TIPO'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_FUMADORasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('FUMADOR_ID', $this->RenderText('FUMADOR ID')));
        ...
        {
            //
            // View column for FUMADOR_ID field
            //
            $column = new TextViewColumn('FUMADOR_ID', 'FUMADOR ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('fumador_id_edit');
            $editColumn = new CustomEditColumn('FUMADOR ID', 'FUMADOR_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_FUMADOR_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_FUMADORGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_FUMADORPage("GE.FUMADOR.php", "GE_FUMADOR", GetCurrentUserGrantForDataSource("GE.FUMADOR"), 'UTF-8');
        $Page->SetShortCaption('GE.FUMADOR');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.FUMADOR');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.FUMADOR"));
