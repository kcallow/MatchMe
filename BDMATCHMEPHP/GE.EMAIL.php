 ...
 */
    ...
    class GE_EMAILPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_EMAILssearch', $this->dataset,
                array('EMAIL', 'LA1', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('EMAIL'), $this->RenderText('USERNAME'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_EMAILasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('GE_EMAILGrid_EMAIL_handler_list');
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('GE_EMAILGrid_EMAIL_handler_view');
        ...
        public function GetModalGridDeleteHandler() { return 'GE_EMAIL_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_EMAILGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('EMAIL', 'EMAIL', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'GE_EMAILGrid_EMAIL_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for EMAIL field
            //
            $column = new TextViewColumn('EMAIL', 'EMAIL', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'GE_EMAILGrid_EMAIL_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new GE_EMAILPage("GE.EMAIL.php", "GE_EMAIL", GetCurrentUserGrantForDataSource("GE.EMAIL"), 'UTF-8');
        $Page->SetShortCaption('GE.EMAIL');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.EMAIL');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.EMAIL"));
