 ...
 */
    ...
    class GE_PARAMETROSPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '"GE"."PARAMETROS"');
            $field = new StringField('PARAMETROS_ID');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_PARAMETROSssearch', $this->dataset,
                array('PARAMETROS_ID', 'VALOR', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('PARAMETROS ID'), $this->RenderText('VALOR'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_PARAMETROSasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('PARAMETROS_ID', $this->RenderText('PARAMETROS ID')));
        ...
        {
            //
            // View column for PARAMETROS_ID field
            //
            $column = new TextViewColumn('PARAMETROS_ID', 'PARAMETROS ID', $this->dataset);
        ...
        {
            //
            // Edit column for PARAMETROS_ID field
            //
            $editor = new TextEdit('parametros_id_edit');
            $editor->SetSize(40);
            $editor->SetMaxLength(40);
            $editColumn = new CustomEditColumn('PARAMETROS ID', 'PARAMETROS_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_PARAMETROS_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_PARAMETROSGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_PARAMETROSPage("GE.PARAMETROS.php", "GE_PARAMETROS", GetCurrentUserGrantForDataSource("GE.PARAMETROS"), 'UTF-8');
        $Page->SetShortCaption('GE.PARAMETROS');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.PARAMETROS');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.PARAMETROS"));
