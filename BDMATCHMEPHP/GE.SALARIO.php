 ...
 */
    ...
    class GE_SALARIOPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_SALARIOssearch', $this->dataset,
                array('SALARIO_ID', 'RANGO', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('SALARIO ID'), $this->RenderText('RANGO'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_SALARIOasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('SALARIO_ID', $this->RenderText('SALARIO ID')));
        ...
        {
            //
            // View column for SALARIO_ID field
            //
            $column = new TextViewColumn('SALARIO_ID', 'SALARIO ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('salario_id_edit');
            $editColumn = new CustomEditColumn('SALARIO ID', 'SALARIO_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_SALARIO_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_SALARIOGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_SALARIOPage("GE.SALARIO.php", "GE_SALARIO", GetCurrentUserGrantForDataSource("GE.SALARIO"), 'UTF-8');
        $Page->SetShortCaption('GE.SALARIO');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.SALARIO');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.SALARIO"));
