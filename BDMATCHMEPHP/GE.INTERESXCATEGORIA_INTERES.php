 ...
 */
    ...
    class GE_INTERESXCATEGORIA_INTERESPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddLookupField('CATEGORIA_INTERES_ID', 'GE.CATEGORIA_INTERES', new IntegerField('CATEGORIA_INTERES_ID'), new StringField('NOMBRE', 'LA2', 'LT2'), 'LT2');
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_INTERESXCATEGORIA_INTERESssearch', $this->dataset,
                array('LA1', 'LA2', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('INTERES ID'), $this->RenderText('CATEGORIA INTERES ID'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_INTERESXCATEGORIA_INTERESasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $lookupDataset->AddField($field, false);
            $lookupDataset->SetOrderBy('NOMBRE', GetOrderTypeAsSQL(otAscending));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateLookupSearchInput('CATEGORIA_INTERES_ID', $this->RenderText('CATEGORIA INTERES ID'), $lookupDataset, 'CATEGORIA_INTERES_ID', 'NOMBRE', false, 8));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            // View column for NOMBRE field
            //
            $column = new TextViewColumn('LA2', 'CATEGORIA INTERES ID', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            $lookupDataset->SetOrderBy('NOMBRE', GetOrderTypeAsSQL(otAscending));
            $editColumn = new LookUpEditColumn(
                'CATEGORIA INTERES ID', 
                'CATEGORIA_INTERES_ID', 
                $editor, 
                $this->dataset, 'CATEGORIA_INTERES_ID', 'NOMBRE', $lookupDataset);
            $validator = new RequiredValidator(StringUtils::Format($this->GetLocalizerCaptions()->GetMessageString('RequiredValidationMessage'), $this->RenderText($editColumn->GetCaption())));
            $editor->GetValidatorCollection()->AddValidator($validator);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_INTERESXCATEGORIA_INTERES_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_INTERESXCATEGORIA_INTERESGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_INTERESXCATEGORIA_INTERESPage("GE.INTERESXCATEGORIA_INTERES.php", "GE_INTERESXCATEGORIA_INTERES", GetCurrentUserGrantForDataSource("GE.INTERESXCATEGORIA_INTERES"), 'UTF-8');
        $Page->SetShortCaption('GE.INTERESXCATEGORIA INTERES');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.INTERESXCATEGORIA INTERES');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.INTERESXCATEGORIA_INTERES"));
