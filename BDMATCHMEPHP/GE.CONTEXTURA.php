 ...
 */
    ...
    class GE_CONTEXTURAPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('GE_CONTEXTURAssearch', $this->dataset,
                array('CONTEXTURA_ID', 'TIPO', 'FEC_CREACION', 'USUARIO_CREACION', 'FEC_ULTIMA_MOD', 'USUARIO_ULTIMA_MOD'),
                array($this->RenderText('CONTEXTURA ID'), $this->RenderText('TIPO'), $this->RenderText('FEC CREACION'), $this->RenderText('USUARIO CREACION'), $this->RenderText('FEC ULTIMA MOD'), $this->RenderText('USUARIO ULTIMA MOD')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('GE_CONTEXTURAasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('CONTEXTURA_ID', $this->RenderText('CONTEXTURA ID')));
        ...
        {
            //
            // View column for CONTEXTURA_ID field
            //
            $column = new TextViewColumn('CONTEXTURA_ID', 'CONTEXTURA ID', $this->dataset);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('contextura_id_edit');
            $editColumn = new CustomEditColumn('CONTEXTURA ID', 'CONTEXTURA_ID', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'GE_CONTEXTURA_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'GE_CONTEXTURAGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new GE_CONTEXTURAPage("GE.CONTEXTURA.php", "GE_CONTEXTURA", GetCurrentUserGrantForDataSource("GE.CONTEXTURA"), 'UTF-8');
        $Page->SetShortCaption('GE.CONTEXTURA');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('GE.CONTEXTURA');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("GE.CONTEXTURA"));
