...
require_once("include/GE_OCUPACIONXCATEGORIA_OCUPACION_variables.php");
...
$page_layouts["GE_OCUPACIONXCATEGORIA_OCUPACION_detailspreview"] = $layout;
...
if($mastertable == "GE.OCUPACION")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("OCUPACION_ID") . "=" . make_db_value("OCUPACION_ID",$_SESSION[$strTableName."_masterkey1"]);
}
if($mastertable == "GE.CATEGORIA_OCUPACION")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("CATEGORIA_OCUPACION_ID") . "=" . make_db_value("CATEGORIA_OCUPACION_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$keylink.="&key2=".runner_htmlspecialchars(rawurlencode(@$data["CATEGORIA_OCUPACION_ID"]));
	
	
	//	OCUPACION_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("OCUPACION_ID", $data, $keylink);
			$row["OCUPACION_ID_value"] = $value;
			$format = $pSet->getViewFormat("OCUPACION_ID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("OCUPACION_ID")))
				$class = ' rnr-field-number';
			$row["OCUPACION_ID_class"] = $class;
	//	CATEGORIA_OCUPACION_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("CATEGORIA_OCUPACION_ID", $data, $keylink);
			$row["CATEGORIA_OCUPACION_ID_value"] = $value;
			$format = $pSet->getViewFormat("CATEGORIA_OCUPACION_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("CATEGORIA_OCUPACION_ID")))
				$class = ' rnr-field-number';
			$row["CATEGORIA_OCUPACION_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_OCUPACIONXCATEGORIA_OCUPACION", "detailspreview"));
