...
$(document).ready(function(){
	...
	$("#addrel_btn").click(function(){
		...
		if ( tbl_left == -1 ) {
			$("#alert").html("<p>'."Seleccione tabla izquierda".'</p>").dialog("open");
			return;
		}
		if ( tbl_right == -1 ) {
			$("#alert").html("<p>'."Seleccione tabla derecha".'</p>").dialog("open");
			return;
		}
		...
		if (relation_stack[rel_txt] != undefined) {
			$("#alert").html("<p>'."La relación con los parámetros seleccionados ya existe".'</p>").dialog("open");
			return;
		}		
	...
	$("#remrel_btn").click(function(){
		...
		if ($("#rel_list").children(":selected").length > 0) {
			...
			$("#alert").html("<p>'."Seleccione la relación que quiere eliminar".'</p>").dialog("open");
			return;
		}
	});
