...
require_once("include/GE_PERSONA_DISPONIBLEXCOLOR_PELO_variables.php");
...
$page_layouts["GE_PERSONA_DISPONIBLEXCOLOR_PELO_detailspreview"] = $layout;
...
if($mastertable == "GE.COLOR_PELO")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("COLOR_PELO_ID") . "=" . make_db_value("COLOR_PELO_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	COLOR_PELO_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("COLOR_PELO_ID", $data, $keylink);
			$row["COLOR_PELO_ID_value"] = $value;
			$format = $pSet->getViewFormat("COLOR_PELO_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("COLOR_PELO_ID")))
				$class = ' rnr-field-number';
			$row["COLOR_PELO_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_PERSONA_DISPONIBLEXCOLOR_PELO", "detailspreview"));
