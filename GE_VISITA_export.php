...
$layout = new TLayout("export2", "Rounded1DeliciousLavender1", "MobileDeliciousLavender1");
$layout->version = 2;
...
$page_layouts["GE_VISITA_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["FEC_VISITA"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["USERNAME_VISITANTE"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$keys["USERNAME_VISITADA"] = refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["FEC_VISITA"] = urldecode($arr[0]);
			$keys["USERNAME_VISITANTE"] = urldecode($arr[1]);
			$keys["USERNAME_VISITADA"] = urldecode($arr[2]);
			$selected_recs[] = $keys;
		}
	}
