...
require_once("include/GE_INTERESXCATEGORIA_INTERES_variables.php");
...
$page_layouts["GE_INTERESXCATEGORIA_INTERES_detailspreview"] = $layout;
...
if($mastertable == "GE.CATEGORIA_INTERES")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("CATEGORIA_INTERES_ID") . "=" . make_db_value("CATEGORIA_INTERES_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$keylink.="&key2=".runner_htmlspecialchars(rawurlencode(@$data["CATEGORIA_INTERES_ID"]));
	...
	//	CATEGORIA_INTERES_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("CATEGORIA_INTERES_ID", $data, $keylink);
			$row["CATEGORIA_INTERES_ID_value"] = $value;
			$format = $pSet->getViewFormat("CATEGORIA_INTERES_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("CATEGORIA_INTERES_ID")))
				$class = ' rnr-field-number';
			$row["CATEGORIA_INTERES_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_INTERESXCATEGORIA_INTERES", "detailspreview"));
