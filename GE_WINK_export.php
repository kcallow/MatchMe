...
$page_layouts["GE_WINK_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["FEC_WINK"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["USERNAME_EMISOR"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$keys["USERNAME_RECEPTOR"] = refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["FEC_WINK"] = urldecode($arr[0]);
			$keys["USERNAME_EMISOR"] = urldecode($arr[1]);
			$keys["USERNAME_RECEPTOR"] = urldecode($arr[2]);
			$selected_recs[] = $keys;
		}
	}
