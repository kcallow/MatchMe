...
require_once("include/GE_WINK_variables.php");
...
$page_layouts["GE_WINK_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("USERNAME_EMISOR") . "=" . make_db_value("USERNAME_EMISOR",$_SESSION[$strTableName."_masterkey1"]);
		$where.=" and ";
	$where .= $pageObject->getFieldSQLDecrypt("USERNAME_RECEPTOR") . "=" . make_db_value("USERNAME_RECEPTOR",$_SESSION[$strTableName."_masterkey2"]);
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["FEC_WINK"]));
		$keylink.="&key2=".runner_htmlspecialchars(rawurlencode(@$data["USERNAME_EMISOR"]));
		$keylink.="&key3=".runner_htmlspecialchars(rawurlencode(@$data["USERNAME_RECEPTOR"]));
	
	
	//	FEC_WINK - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("FEC_WINK", $data, $keylink);
			$row["FEC_WINK_value"] = $value;
			$format = $pSet->getViewFormat("FEC_WINK");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("FEC_WINK")))
				$class = ' rnr-field-number';
			$row["FEC_WINK_class"] = $class;
	//	USERNAME_EMISOR - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("USERNAME_EMISOR", $data, $keylink);
			$row["USERNAME_EMISOR_value"] = $value;
			$format = $pSet->getViewFormat("USERNAME_EMISOR");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("USERNAME_EMISOR")))
				$class = ' rnr-field-number';
			$row["USERNAME_EMISOR_class"] = $class;
	//	USERNAME_RECEPTOR - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("USERNAME_RECEPTOR", $data, $keylink);
			$row["USERNAME_RECEPTOR_value"] = $value;
			$format = $pSet->getViewFormat("USERNAME_RECEPTOR");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("USERNAME_RECEPTOR")))
				$class = ' rnr-field-number';
			$row["USERNAME_RECEPTOR_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_WINK", "detailspreview"));
