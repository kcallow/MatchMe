...
require_once("include/GE_EVENTO_variables.php");
...
$page_layouts["GE_EVENTO_detailspreview"] = $layout;
...
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("CIUDAD") . "=" . make_db_value("CIUDAD",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["EVENTO_ID"]));
	
	
	//	EVENTO_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("EVENTO_ID", $data, $keylink);
			$row["EVENTO_ID_value"] = $value;
			$format = $pSet->getViewFormat("EVENTO_ID");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("EVENTO_ID")))
				$class = ' rnr-field-number';
			$row["EVENTO_ID_class"] = $class;
	//	NOMBRE - 
			$viewContainer->recId = $recordsCounter;
	...
	//	FECHA_HORA - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("FECHA_HORA", $data, $keylink);
			$row["FECHA_HORA_value"] = $value;
			$format = $pSet->getViewFormat("FECHA_HORA");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("FECHA_HORA")))
				$class = ' rnr-field-number';
			$row["FECHA_HORA_class"] = $class;
	//	LUGAR - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("LUGAR", $data, $keylink);
			$row["LUGAR_value"] = $value;
			$format = $pSet->getViewFormat("LUGAR");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("LUGAR")))
				$class = ' rnr-field-number';
			$row["LUGAR_class"] = $class;
	//	DESCRIPCION - 
			$viewContainer->recId = $recordsCounter;
	...
	//	CIUDAD - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("CIUDAD", $data, $keylink);
			$row["CIUDAD_value"] = $value;
			$format = $pSet->getViewFormat("CIUDAD");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("CIUDAD")))
				$class = ' rnr-field-number';
			$row["CIUDAD_class"] = $class;
		$rowinfo[] = $row;
		if ($b) {
			$rowinfo2[] = $row;
			$b = false;
...
$xt->load_template(GetTemplateName("GE_EVENTO", "detailspreview"));
