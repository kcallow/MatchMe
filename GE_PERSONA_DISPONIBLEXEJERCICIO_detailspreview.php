...
require_once("include/GE_PERSONA_DISPONIBLEXEJERCICIO_variables.php");
...
$page_layouts["GE_PERSONA_DISPONIBLEXEJERCICIO_detailspreview"] = $layout;
...
if($mastertable == "GE.EJERCICIO")
{
	$where = "";
		$where .= $pageObject->getFieldSQLDecrypt("EJERCICIO_ID") . "=" . make_db_value("EJERCICIO_ID",$_SESSION[$strTableName."_masterkey1"]);
}
...
if($rowcount) 
{
	...
	//	EJERCICIO_ID - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("EJERCICIO_ID", $data, $keylink);
			$row["EJERCICIO_ID_value"] = $value;
			$format = $pSet->getViewFormat("EJERCICIO_ID");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("EJERCICIO_ID")))
				$class = ' rnr-field-number';
			$row["EJERCICIO_ID_class"] = $class;
...
$xt->load_template(GetTemplateName("GE_PERSONA_DISPONIBLEXEJERCICIO", "detailspreview"));
