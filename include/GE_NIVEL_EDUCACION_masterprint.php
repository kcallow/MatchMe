<?php
function DisplayMasterTableInfoForPrint_GE_NIVEL_EDUCACION($params)
{
	global $cman;
	...
	{
		$keysAssoc["NIVEL_EDUCACION_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("NIVEL_EDUCACION_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("NIVEL_EDUCACION_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("NIVEL_EDUCACION_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_NIVEL_EDUCACION","NIVEL_EDUCACION_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["NIVEL_EDUCACION_ID"]));
	...
	$xt->assign("GRADO_mastervalue", $viewControls->showDBValue("GRADO", $data, $keylink));
	$format = $settings->getViewFormat("GRADO");
	$class = " rnr-field-text";
	if($format == FORMAT_FILE) 
		$class = ' rnr-field-file'; 
	if($format == FORMAT_AUDIO)
	...
	if($format == FORMAT_CHECKBOX)
		$class = ' rnr-field-checkbox';
	if($format == FORMAT_NUMBER || IsNumberType($settings->getFieldType("GRADO")))
		$class = ' rnr-field-number';
		
	$xt->assign("GRADO_class", $class); // add class for field header as field value
	...
	$layout = GetPageLayout("GE_NIVEL_EDUCACION", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_NIVEL_EDUCACION", "masterprint"));
}
