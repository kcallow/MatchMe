<?php
function DisplayMasterTableInfoForPrint_GE_CATEGORIA_OCUPACION($params)
{
	global $cman;
	...
	{
		$keysAssoc["CATEGORIA_OCUPACION_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("CATEGORIA_OCUPACION_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("CATEGORIA_OCUPACION_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("CATEGORIA_OCUPACION_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_CATEGORIA_OCUPACION","CATEGORIA_OCUPACION_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["CATEGORIA_OCUPACION_ID"]));
	
	$xt->assign("CATEGORIA_OCUPACION_ID_mastervalue", $viewControls->showDBValue("CATEGORIA_OCUPACION_ID", $data, $keylink));
	$format = $settings->getViewFormat("CATEGORIA_OCUPACION_ID");
	...
	if($format == FORMAT_NUMBER || IsNumberType($settings->getFieldType("CATEGORIA_OCUPACION_ID")))
		$class = ' rnr-field-number';
		
	$xt->assign("CATEGORIA_OCUPACION_ID_class", $class); // add class for field header as field value
	...
	$layout = GetPageLayout("GE_CATEGORIA_OCUPACION", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_CATEGORIA_OCUPACION", "masterprint"));
}
