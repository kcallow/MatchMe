<?php
function DisplayMasterTableInfoForPrint_GE_COLOR_OJOS($params)
{
	global $cman;
	...
	{
		$keysAssoc["COLOR_OJOS_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("COLOR_OJOS_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("COLOR_OJOS_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("COLOR_OJOS_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_COLOR_OJOS","COLOR_OJOS_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["COLOR_OJOS_ID"]));
	...
	$layout = GetPageLayout("GE_COLOR_OJOS", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_COLOR_OJOS", "masterprint"));
}
