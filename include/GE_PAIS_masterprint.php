<?php
function DisplayMasterTableInfoForPrint_GE_PAIS($params)
{
	global $cman;
	...
	if( $detailtable == "GE.CIUDAD" )
	{
		$keysAssoc["PAIS_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("PAIS_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("PAIS_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("PAIS_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_PAIS","PAIS_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["PAIS_ID"]));
	...
	$layout = GetPageLayout("GE_PAIS", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_PAIS", "masterprint"));
}
