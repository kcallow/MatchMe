<?php
function DisplayMasterTableInfoForPrint_GE_RELIGION($params)
{
	global $cman;
	...
	{
		$keysAssoc["RELIGION_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("RELIGION_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("RELIGION_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("RELIGION_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_RELIGION","RELIGION_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["RELIGION_ID"]));
	
	$xt->assign("RELIGION_ID_mastervalue", $viewControls->showDBValue("RELIGION_ID", $data, $keylink));
	$format = $settings->getViewFormat("RELIGION_ID");
	...
	if($format == FORMAT_NUMBER || IsNumberType($settings->getFieldType("RELIGION_ID")))
		$class = ' rnr-field-number';
		
	$xt->assign("RELIGION_ID_class", $class); // add class for field header as field value
	...
	$layout = GetPageLayout("GE_RELIGION", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_RELIGION", "masterprint"));
}
