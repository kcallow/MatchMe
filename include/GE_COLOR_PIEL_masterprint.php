<?php
function DisplayMasterTableInfoForPrint_GE_COLOR_PIEL($params)
{
	global $cman;
	...
	{
		$keysAssoc["COLOR_PIEL_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("COLOR_PIEL_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("COLOR_PIEL_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("COLOR_PIEL_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_COLOR_PIEL","COLOR_PIEL_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["COLOR_PIEL_ID"]));
	...
	$layout = GetPageLayout("GE_COLOR_PIEL", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_COLOR_PIEL", "masterprint"));
}
