<?php
function DisplayMasterTableInfo_GE_BEBEDOR($params)
{
	global $cman;
	...
	{
		$keysAssoc["BEBEDOR_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("BEBEDOR_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("BEBEDOR_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("BEBEDOR_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_BEBEDOR","BEBEDOR_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["BEBEDOR_ID"]));
	...
	$layout = GetPageLayout("GE_BEBEDOR", 'masterlist');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');
	
	$xt->displayPartial(GetTemplateName("GE_BEBEDOR", "masterlist"));
}
