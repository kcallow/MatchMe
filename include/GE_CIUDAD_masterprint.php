<?php
function DisplayMasterTableInfoForPrint_GE_CIUDAD($params)
{
	global $cman;
	...
	if( $detailtable == "GE.EVENTO" )
	{
		$keysAssoc["CIUDAD_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("CIUDAD_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("CIUDAD_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("CIUDAD_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_CIUDAD","CIUDAD_ID").": ".$keyValue;
		$xt->assign('showKeys', $showKeys);	
	}

	if( $detailtable == "GE.PERSONA" )
	{
		$keysAssoc["CIUDAD_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("CIUDAD_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("CIUDAD_ID", $keys[1-1], "", true);
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["CIUDAD_ID"]));
	...
	$xt->assign("PAIS_ID_mastervalue", $viewControls->showDBValue("PAIS_ID", $data, $keylink));
	$format = $settings->getViewFormat("PAIS_ID");
	$class = " rnr-field-text";
	if($format == FORMAT_FILE) 
		$class = ' rnr-field-file'; 
	if($format == FORMAT_AUDIO)
	...
	if($format == FORMAT_CHECKBOX)
		$class = ' rnr-field-checkbox';
	if($format == FORMAT_NUMBER || IsNumberType($settings->getFieldType("PAIS_ID")))
		$class = ' rnr-field-number';
		
	$xt->assign("PAIS_ID_class", $class); // add class for field header as field value
	...
	$layout = GetPageLayout("GE_CIUDAD", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_CIUDAD", "masterprint"));
}
