<?php
function DisplayMasterTableInfoForPrint_GE_CONTEXTURA($params)
{
	global $cman;
	...
	{
		$keysAssoc["CONTEXTURA_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("CONTEXTURA_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("CONTEXTURA_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("CONTEXTURA_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_CONTEXTURA","CONTEXTURA_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["CONTEXTURA_ID"]));
	...
	$layout = GetPageLayout("GE_CONTEXTURA", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_CONTEXTURA", "masterprint"));
}
