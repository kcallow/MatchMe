<?php
function DisplayMasterTableInfoForPrint_GE_GENERO($params)
{
	global $cman;
	...
	{
		$keysAssoc["GENERO_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("GENERO_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("GENERO_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("GENERO_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_GENERO","GENERO_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["GENERO_ID"]));
	...
	$layout = GetPageLayout("GE_GENERO", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_GENERO", "masterprint"));
}
