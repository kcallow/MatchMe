<?php
function DisplayMasterTableInfoForPrint_GE_EJERCICIO($params)
{
	global $cman;
	...
	{
		$keysAssoc["EJERCICIO_ID"] = $keys[1-1];
				$where.= RunnerPage::_getFieldSQLDecrypt("EJERCICIO_ID", $connection , $settings , $cipherer) . "=" . $cipherer->MakeDBValue("EJERCICIO_ID", $keys[1-1], "", true);

				$keyValue = $viewControls->showDBValue("EJERCICIO_ID", $keysAssoc);
		$showKeys.= " ".GetFieldLabel("GE_EJERCICIO","EJERCICIO_ID").": ".$keyValue;
	...
	$keylink = "";
	$keylink.= "&key1=".runner_htmlspecialchars(rawurlencode(@$data["EJERCICIO_ID"]));
	
	$xt->assign("EJERCICIO_ID_mastervalue", $viewControls->showDBValue("EJERCICIO_ID", $data, $keylink));
	$format = $settings->getViewFormat("EJERCICIO_ID");
	...
	if($format == FORMAT_NUMBER || IsNumberType($settings->getFieldType("EJERCICIO_ID")))
		$class = ' rnr-field-number';
		
	$xt->assign("EJERCICIO_ID_class", $class); // add class for field header as field value
	...
	$layout = GetPageLayout("GE_EJERCICIO", 'masterprint');
	if( $layout )
		$xt->assign("pageattrs", 'class="'.$layout->style." page-".$layout->name.'"');

	$xt->displayPartial(GetTemplateName("GE_EJERCICIO", "masterprint"));
}
